...
var Canvas = function () {
	...
	_createClass(Canvas, [{
		...
		value: function resize() {
			...
			ground = this.height > 500 ? /* 0.85 */1 : 1.0;
		}
	}]);
	return Canvas;
}();
...
function dancerpointdraw() {
	...
	for (var i = 0; i < Thedancer[0].length; i++) {
		if ((Thedancer[0][i].x) > canvas.width || (Thedancer[0][i].x) == canvas.width / 2 || (Thedancer[0][i].x) == canvas.width / 3 || (Thedancer[0][i].x) == Math.random() * canvas.width || (Thedancer[0][i].y) > canvas.height) {
			Thedancer[0][i].x = Thedancer[0][i].x - 1
			Thedancer[0][i].y = Thedancer[0][i].y - 1
		}
		...
		// ctx.arc(Thedancer[0][11].x, Thedancer[0][11].y, 5, 0, 2 * Math.PI);
		// ctx.fill();
	...
	ctx.arc(Thedancer[0][0].x - 4, Thedancer[0][0].y - 5, 1, 0, 2 * Math.PI);
	ctx.arc(Thedancer[0][0].x + 4, Thedancer[0][0].y - 5, 1, 0, 2 * Math.PI);
	ctx.fill();
	...
	// var grd;
	ctx.lineWidth = 10;
	var grd = ctx.createLinearGradient(Thedancer[0][0].x, Thedancer[0][0].y, Thedancer[0][11].x, Thedancer[0][11].y);
	grd.addColorStop(0, "white");
	grd.addColorStop(1, "RGBA(255,0,0,1)");
	ctx.strokeStyle = grd;
	ctx.beginPath()
	// grd = ctx.createLinearGradient(Thedancer[0][0].x, Thedancer[0][0].y, /* 1, */ Thedancer[0][11].x, Thedancer[0][11].y/* , 1 */);
	ctx.moveTo(Thedancer[0][0].x, Thedancer[0][0].y);
	ctx.lineTo(Thedancer[0][11].x, Thedancer[0][11].y);
	ctx.stroke()
...
function run() {
	...
	try {
		for (var _iterator9 = dancers[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next())
			...
			Thedancer.id = /* dancerId */ Math.floor(Math.random() * noOfDancers)
			dancerpointdraw()
...
function correctGround() {
	if (ground !== 1) {
		var grndDistance = (1 - ground) / 30;
		ground += grndDistance;
